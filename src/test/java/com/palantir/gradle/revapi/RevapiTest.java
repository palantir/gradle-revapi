...
class RevapiTest {
    ...
    class SimpleSingleProject {
        ...
        @Test
        void fails_when_comparing_produced_jar_versus_some_random_other_jar(
        ...
        void revapiAcceptAllBreaks_succeeds_when_there_are_no_breaking_changes(
            ...
            gradle.withArgs("revapiAcceptAllBreaks", "--justification", "fight me")
                    .buildsSuccessfully();
        }
        ...
        void does_not_error_out_when_project_has_a_version_greater_than_the_old_version(
            ...
            rootProject.settingsGradle().rootProjectName(revApiProjectName);
            ...
            runRevapiExpectingToFindDifferences(gradle, rootProject, revApiProjectName);
        }
        ...
        @Test
        void skips_revapi_tasks_when_the_versions_to_check_is_empty_list(
                GradleInvoker gradle, RootProject rootProject) {
            rootProject.buildGradle().append("""
                revapi {
                    oldGroup = 'org.revapi'
    ...
    class UnpublishedLibrary {
        ...
        @Test
        void revapi_doesnt_fail(GradleInvoker gradle) {
            InvocationResult result = gradle.withArgs("revapi").buildsSuccessfully();
            assertThat(result).task(":revapiAnalyze").skipped();
            assertThat(result).task(":revapi").skipped();
        }
        ...
        @Test
        void revapiAcceptAllBreaks_is_a_no_op(GradleInvoker gradle) {
        ...
        @Test
        void revapiAcceptBreak_doesnt_fail(GradleInvoker gradle) {
    ...
    @Nested
    class TwoSubProjectsWithPublishing {

        @BeforeEach
        void setUp(RootProject rootProject, MavenRepo repo, SubProject one, SubProject two) {
            rootProject.buildGradle().append("""
                allprojects {
                    group = 'revapi.test'
                    version = '1.0.0'
            ...
            one.buildGradle().plugins().add("java-library").add("maven-publish");
            one.buildGradle().append(testMavenPublication(repo));

            two.buildGradle().plugins().add("java-library").add("maven-publish");
            two.buildGradle().append(testMavenPublication(repo));
        ...
        void moving_a_class_from_one_project_to_a_dependent_project_is_not_a_break(
                GradleInvoker gradle, SubProject one, SubProject two) throws IOException {
        ...
        @Test
        void ignores_breaks_in_dependent_projects(GradleInvoker gradle, SubProject one, SubProject two) {
    ...
    class GroovyProjects {
        ...
        @Test
        void detects_breaks_in_groovy_code(GradleInvoker gradle, RootProject rootProject) {
